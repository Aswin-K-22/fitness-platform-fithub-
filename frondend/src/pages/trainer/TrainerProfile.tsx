...
const getValidationSchema = (hasExistingProfilePic: boolean) => {
  return Yup.object().shape({
    ...
    profilePic: hasExistingProfilePic
      ? Yup.mixed<File>()
          .nullable()
          .test(
            "fileFormat",
      ...
      : Yup.mixed<File>()
          .required("Profile picture is required for first-time setup")
          .test(
            "fileFormat",
            "Only JPEG or PNG images are allowed",
            function (value): boolean {
...
const ProfileImagePlaceholder = () => (
  <div className="h-24 w-24 sm:h-32 sm:w-32 rounded-full border-4 border-white bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
    <div className="text-center">
      <svg className="w-12 h-12 sm:w-16 sm:h-16 mx-auto text-gray-400 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const TrainerProfile: React.FC = () => {
 ...
 const hasExistingProfilePic = !!profileData?.profilePic || !!trainer?.profilePic;

  // Create validation schema based on whether profile picture exists
  const validationSchema = getValidationSchema(hasExistingProfilePic);
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        console.log("Fetched profile:", trainerProfile);
        console.log("ProfilePic from API:", trainerProfile.profilePic);
        console.log("ProfilePic from Redux:", trainer?.profilePic);
        ...
        // Use trainerProfile.profilePic directly (full S3 URL) or fallback to trainer.profilePic
        const profilePicUrl = trainerProfile.profilePic || trainer?.profilePic || null;
        console.log("Setting previewUrl:", profilePicUrl);
        setPreviewUrl(profilePicUrl);
        // Sync Redux store with API data
        if (trainer && trainerProfile.profilePic) {
          dispatch(setAuth({
            trainer: { ...trainer, profilePic: trainerProfile.profilePic },
...
const handleSave = useCallback(async () => {
  ...
  try {
    const updateData: IUpdateTrainerProfileRequestDTO = {
      name: editedData.name || profileData?.name || "",
      bio: editedData.bio || profileData?.bio || "",
      specialties: editedData.specialties.length > 0 ? editedData.specialties : profileData?.specialties || [],
      profilePic: editedData.profilePic || undefined,
      upiId: editedData.upiId || profileData?.paymentDetails?.upiId || "",
      bankAccount: editedData.bankAccount || profileData?.paymentDetails?.bankAccount || "",
      ifscCode: editedData.ifscCode || profileData?.paymentDetails?.ifscCode || "",
    };
             ...
             {previewUrl || profileData.profilePic ? (
                <img
                  ...
                  onError={(e) => {
                    ...
                    if (parent) {
                      parent.innerHTML = `
                        <div class="h-24 w-24 sm:h-32 sm:w-32 rounded-full border-4 border-white bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
                          <div class="text-center">
                            <svg class="w-12 h-12 sm:w-16 sm:h-16 mx-auto text-gray-400 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <p class="text-xs sm:text-sm text-gray-500 font-medium">No Image</p>
                          </div>
                        </div>
                      `;
                    }
